...
<?php $empleado = $empleado ?? ['id' => '', 'clave' => '', 'nombre' => '', 'fecha_nacimiento' => '', 'telefono' => '', 'email' => '', 'id_departamento' => ''] ?>
<?php $rutaBase = $rutaBase ?? 'es/empleados' ?>

<?= $this->section('contenido') ?>
<div class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="h3 mb-0"><?= esc(lang('App.editEmployee')) ?></h1>
...
<div class="card shadow-sm">
    <div class="card-body">
        <form action="<?= base_url($rutaBase . '/' . $empleado['id']) ?>" method="post">
            <?= csrf_field() ?>
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control" id="clave" name="clave" value="<?= old('clave', $empleado['clave']) ?>" maxlength="10">
                </div>

                <div class="col-md-8">
                    <label for="nombre" class="form-label"><?= esc(lang('App.name')) ?></label>
                    <input type="text" class="form-control" id="nombre" name="nombre" value="<?= old('nombre', $empleado['nombre']) ?>" maxlength="100">
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control" id="telefono" name="telefono" value="<?= old('telefono', $empleado['telefono']) ?>" maxlength="20">
                </div>

                <div class="col-md-4">
                    <label for="email" class="form-label"><?= esc(lang('App.email')) ?></label>
                    <input type="email" class="form-control" id="email" name="email" value="<?= old('email', $empleado['email']) ?>" maxlength="100">
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select" id="id_departamento" name="id_departamento">
                        ...
                        <?php foreach ($departamentos as $departamento): ?>
                            <option value="<?= esc((string) $departamento['id']) ?>" <?= old('id_departamento', $empleado['id_departamento']) == $departamento['id'] ? 'selected' : '' ?>>
            ...
            <div class="mt-4">
                <button type="submit" class="btn btn-primary"><?= esc(lang('App.update')) ?></button>
            </div>
        </form>
    </div>
</div>

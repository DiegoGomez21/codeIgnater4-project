...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="h3 mb-0">{lang key='App.newEmployee'}</h1>
...
<div class="card shadow-sm">
    <div class="card-body">
        <form action="{base_url uri=$rutaBase}" method="post">
            {csrf_field}
                    ...
                    <select class="form-select" id="id_departamento" name="id_departamento">
                        ...
                        {foreach $departamentos as $departamento}
                            <option value="{$departamento.id|escape}"
                                {old_select key='id_departamento' value=$departamento.id current=''}>
            ...
            <div class="mt-4">
                <button type="submit" class="btn btn-primary">{lang key='App.save'}</button>
            </div>
        </form>
    </div>
</div>
